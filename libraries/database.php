<?php 
    ...
    */
    class Database
    {
        ...
        {
            $this->link = mysqli_connect("localhost","root","","milkyshop") or die ();
         ...
         * @return integer
         */
        // truyen 2 tham so ten bang va data
        ...
        /*
            Gồm 3 tham số: bảng, mảng dữ liệu, điều kiện(mảng   )
        */
        ...
        public function updatesql($table, $sql) {
        ...
        public function countElement($element, $table, $conditions) {
            $sql = "SELECT {$element} FROM {$table} WHERE {$conditions}";
            $result = mysqli_query($this->link, $sql) or die("Lỗi Truy Vấn countTable----" .mysqli_error($this->link));
            $num = mysqli_num_rows($result);
            return $num;
        }
        ...
        public function deletequery ($table ,  $sql )
        {
            $sql = "DELETE FROM {$table} WHERE " .$sql;
            // _debug($sql);die;
            $result = mysqli_query($this->link,$sql) or die (" Lỗi Truy Vấn delete   --- " .mysqli_error($this->link));
            // return mysqli_affected_rows($this->link);
            return $result;
        }

        public function fetchArrNoCondition($table) {
            // $data = [];
            $sql = "SELECT * FROM {$table}";
            $run = mysqli_query($this->link, $sql);
            $row = mysqli_fetch_array($run);
            
            return $row;
        }

        public function fetchArrWithCondition($sql, $table) {
            // $data = [];
        ...
        {
            // lay du lieu toan bo cua bang do
        ...
        //them cai so trang pagi nen cuoi cau len phai huy (= true)
